...
export default function TechnicianLogin() {
  ...
  const submit = async (e) => {
    ...
    setLoading(true)
    try {
      const userData = await login(email, password, 'technician')
      if (userData.status === 'pending') {
        navigate('/tecnico/aguardando')
      } else if (userData.status === 'rejected') {
        setError('Seu cadastro foi reprovado. Entre em contato com suporte@uaifix.com.br para mais informações.')
      } else {
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="max-w-sm w-full">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-500 text-sm mt-1">Área do Técnico</p>
        </div>
        ...
        <form onSubmit={submit} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-accent-500 text-white rounded-xl py-3 text-sm font-semibold hover:bg-accent-600 transition disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {loading ? (
          ...
          <p className="text-center text-xs text-gray-500">
            Ainda não tem conta?{' '}
            <Link to="/tecnico/cadastro" className="text-primary-700 font-medium hover:underline">
              Cadastrar
            </Link>
          </p>
        </form>
